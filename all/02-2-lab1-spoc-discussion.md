...
>对ucore来说，系统调用的函数全部都在lab8_result/kern/syscall/syscall.c里面，如果想获得一个系统调用的调用次信息，可以在该系统调用函数里面加上计数和显示功能;如果想获得所有系统调用函数的调用次数信息，可以在syscall.c里面设置一个static变量，每次调用都进技术并显示，就可以得到所有的调用次数信息。当然，我觉得应该也可以使用strace来跟踪整个过程，查看相关的系统调用信息。
