...
```
可以看到其执行时间。
>上面体现内存访问局部性好的例子的执行时间为0m0.043s.下面是一个体现内存访问局部性差的简单应用程序例子，其执行时间为0m0.147s。
```
#include <stdio.h>
#define NUM 1024
#define COUNT 10
...
>首先向操作系统报告异常，将控制权交给操作系统内核，然后操作系统查找外存中的页面，将页面换入空闲的页面，空闲页面的换入需要考虑修改位，驻留位，访问位等，然后再修改页表项，重新执行缺页异常处理。
...
```
>程序见[04_1_spoc.cpp](https://github.com/ZhangMenghao/os_exercises/blob/master/all/04_1_spoc.cpp)
```
Virtual Address 6653:
